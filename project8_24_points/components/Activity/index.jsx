...
class Activity extends Component {
  ...
  renderActivityIcon(activityType, link) {
    const iconStyle = { cursor: "pointer", color: "#6b4652" };
    switch (activityType) {
      case "Photo Upload":
  ...
  renderActivityItem(activity) {
    ...
    return (
      <ListItem key={activity._id}>
        ...
        <ListItemText
          primary={(
            <Link to={link} style={{ textDecoration: "none", color: "#e27396" }}>
        ...
        />
        <Link to={`/photos/${activity.user_id._id}`} style={{ textDecoration: "none", color: "#e27396" }}>
          {activity.photo_id?.file_name && (
            <img src={`/images/${activity.photo_id.file_name}`} alt="Thumbnail" style={{ width: 50, height: 50, marginLeft: "auto", borderWidth: "5px", borderRadius: "10px", borderColor: "#e27396", borderStyle: "double"}} />
          )}
        </Link>
      </ListItem>
    );
    ...
    return (
      <Container>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
          ...
          <IconButton 
            variant="contained" 
            style={{ marginBottom: "20px", backgroundColor: "#e27396", color: "#fff" }} 
        ...
        {loading ? (
          <CircularProgress style={{ color: "#ea9ab2" }} />
        ) : (
          <List>
            {activities.length === 0 ? (
              <Typography variant="body1" style={{ color: "#6b4652" }}>Ямар нэг үйл явдал болоогүй байна.</Typography>
            ) : (
