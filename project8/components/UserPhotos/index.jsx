...
class UserPhotos extends React.Component {
  ...
  renderComment = (commentText) => {
    ...
    return commentText.replace(/@\[(.+?)\]\((.+?)\)/g, (match, displayName, userId) => {
      return `<a href="http://localhost:3000/photo-share.html#/users/${userId}" style="text-decoration: none; color: #174069;"><strong>${displayName}</strong></a>`;
    });
  };
        ...
        {this.state.photos.map((photo) => (
          <Grid item xs={12} sm={6} md={4} key={photo._id}>
            ...
            >
              <CardHeader
                avatar={(
                  <Avatar style={{ backgroundColor: "#92c5d1", color: "#fff" }}>
                    {`${this.state.user.first_name[0].toUpperCase()}${this.state.user.last_name[0].toUpperCase()}`}
                  </Avatar>
                )}
                title={(
                  <Link to={`/users/${this.state.user._id}`} style={{ textDecoration: "none", color: "#11262b" }}>
              ...
              <CardContent>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
                  <Button
                    ...
                    sx={{
                      // text color
                      color: photo.likes.includes(this.props.loginUser._id) ? "#fff": "#92c5d1",
                      // background color
                      backgroundColor: photo.likes.includes(this.props.loginUser._id) ? "#92c5d1" : "transparent",
                      borderColor: "#92c5d1",
                      "&:hover": {
                        backgroundColor: photo.likes.includes(this.props.loginUser._id) ? "#2060a1" : "rgba(226, 115, 150, 0.1)",
                        borderColor: "#2060a1",
                      },
                      "&:active": {
                        backgroundColor: photo.likes.includes(this.props.loginUser._id) ? "#2060a1" : "rgba(226, 115, 150, 0.2)",
                        borderColor: "#2060a1",
                      },
                    }}
                    onClick={() => {
                  ...
                  <Typography variant="body2" style={{ marginLeft: "10px", color: "#11262b" }}>
                ...
                {photo.comments && (
                  <Typography variant="subtitle1" gutterBottom style={{ color: "#11262b" }}>
                    Сэтгэгдлүүд:
                  </Typography>
                )}
                ...
                {photo.comments.map((c) => (
                  <List key={c._id}>
                    <Typography variant="subtitle2" style={{ color: "#92c5d1" }}>
                      <Link to={`/users/${c.user._id}`} style={{ textDecoration: "none", color: "#11262b" }}>
                    ...
                    <Divider style={{ backgroundColor: "#92c5d1" }} />
                  </List>
                ))}
                <CommentDialog
