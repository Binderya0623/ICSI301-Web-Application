...
export default class UserPhotos extends React.Component {
  ...
  renderComment = (commentText) => {
    ...
    return commentText.replace(/@\[(.+?)\]\((.+?)\)/g, (match, displayName, userId) => {
      return `<a href="http://localhost:3000/photo-share.html#/users/${userId}" style="text-decoration: none; color: red;">${displayName}</a>`;
    });
  };
              ...
              <CardContent>
                ...
                {photo.comments.map((c) => (
                  <List key={c._id}>
                    ...
                    <Typography variant="body1" dangerouslySetInnerHTML={{ __html: this.renderComment(c.comment) }} />
